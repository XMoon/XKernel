...
static int rndis_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	mapphone_usb_interface_enum_cb(RNDIS_TYPE_FLAG);
...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	status = mapphone_usb_interface_id(c, f);
	if (status < 0)
		goto fail;
	...
	ep = mapphone_usb_ep_autoconfig(cdev->gadget, &fs_in_desc);
	if (!ep)
		goto fail;
	rndis->port.in_ep = ep;
	ep->driver_data = cdev;	/* claim */

	ep = mapphone_usb_ep_autoconfig(cdev->gadget, &fs_out_desc);
	if (!ep)
		goto fail;
	 ...
	 */
	ep = mapphone_usb_ep_autoconfig(cdev->gadget, &fs_notify_desc);
	if (!ep)
		goto fail;
	rndis->notify = ep;
	ep->driver_data = cdev;	/* claim */
	...
	f->descriptors = mapphone_usb_copy_descriptors(eth_fs_function);
	
	if (!f->descriptors)
		goto fail;

	rndis->fs.in = mapphone_usb_find_endpoint(eth_fs_function,
			f->descriptors, &fs_in_desc);
	rndis->fs.out = mapphone_usb_find_endpoint(eth_fs_function,
			f->descriptors, &fs_out_desc);
	rndis->fs.notify = mapphone_usb_find_endpoint(eth_fs_function,
	...
	if (gadget_is_dualspeed(c->cdev->gadget)) {
		...
		/* copy descriptors, and track endpoint copies */
		f->hs_descriptors = mapphone_usb_copy_descriptors(eth_hs_function);

		if (!f->hs_descriptors)
			goto fail;

		rndis->hs.in = mapphone_usb_find_endpoint(eth_hs_function,
				f->hs_descriptors, &hs_in_desc);
		rndis->hs.out = mapphone_usb_find_endpoint(eth_hs_function,
				f->hs_descriptors, &hs_out_desc);
		rndis->hs.notify = mapphone_usb_find_endpoint(eth_hs_function,
...
int __init rndis_bind_config(struct usb_configuration *c, u8 ethaddr[ETH_ALEN])
{
	...
	if (rndis_string_defs[0].id == 0) {
		...
		status = mapphone_usb_string_id(c->cdev);
		if (status < 0)
			return status;
	...
	status = mapphone_usb_add_function(c, &rndis->port.func);
	if (status) {
		kfree(rndis);
fail:
		rndis_exit();
...
struct android_usb_function* __init mapphone_f_rndis_init(void)
{
	printk(KERN_INFO "f_rndis init\n");
	platform_driver_register(&rndis_platform_driver);

	return &rndis_function;
}
